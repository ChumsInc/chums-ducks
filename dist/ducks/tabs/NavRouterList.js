...
var NavList = function (_a) {
    ...
    return (React.createElement("ul", { className: classNames('nav', className) },
        list.map(function (tab) { return (React.createElement(NavRouterLink, { to: tab.to || '', key: tab.id, id: tab.id, title: tab.title, className: itemClassName, disabled: tab.disabled, active: tab.id === selected, canClose: tab.canClose, onClose: function () { return tabCloseHandler(tab.id); } })); }),
        children));
};
